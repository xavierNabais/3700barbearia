...
export default {
  ...
  methods: {
    async fetchUtilizadores() {
      try {
        const response = await fetch('https://3700barbearia-api.vercel.app/painel/utilizadores');
    ...
    async excluirUtilizador(userIdToDelete) {
      try {
        const response = await fetch(`https://3700barbearia-api.vercel.app/painel/utilizadores/${userIdToDelete}`, {
          method: 'DELETE'
        });
    ...
    closeCreatePopup(){
      ...
      if (this.editedUser.Password.trim() == '') {
        ...
        try {
          ...
          const response = await fetch(`https://3700barbearia-api.vercel.app/painel/utilizadores/${this.userIdToEdit}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json'
            },

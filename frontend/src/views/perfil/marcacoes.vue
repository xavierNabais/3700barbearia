...
methods: {
  ...
  async procurarAnteriores() {
    ...
    await this.fetchMarcacoes(`https://3700barbearia-api.vercel.app/perfil/marcacoes/anteriores/${userId}`);
  ...
  async procurarProximas() {
    ...
    const decoded = jwtDecode(token);
    const userId = decoded.userId;
    await this.fetchMarcacoes(`https://3700barbearia-api.vercel.app/perfil/marcacoes/proximas/${userId}`);
